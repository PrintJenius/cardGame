...
function cardClick(card) { 
    ...
    if (visiblefrontCardNum() === 26) { // 클릭해서 뒤집은 카드가 전부 앞면일때
        clearInterval(timeInterval);
        setTimeout(function () {
...
function getCurrentStageFromURL() {
    ...
    const stageNumber = url.match(/hellStage(\d+)/);
...
function clearStage() {
    ...
    if (!hellStageProgress[currentStage]) {
        hellStageProgress[currentStage] = true;
        localStorage.setItem('hellStageProgress', JSON.stringify(hellStageProgress));
    }
